...
from gflownet.tasks.circuit_task import QuantumCircuitTask, QuantumCircuitTrainer
...
def circuit2depth(circuits: List[QuantumCircuit], is_valid: List[bool], default=100):
    """Compute circuit depth as an objective (lower is better)"""
    depths = []
    for circuit, valid in zip(circuits, is_valid):
        if not valid:
        ...
        for gate in circuit.nodes:
            if circuit.nodes[gate].get("type") == "gate":
                for qubit in circuit.get_gate_qubits(gate):
                    qubit_depths[qubit] += 1
        depths.append(max(qubit_depths) if qubit_depths else default)
    return torch.tensor(depths)


def circuit2gate_count(circuits: List[QuantumCircuit], is_valid: List[bool], default=100):
    """Compute total gate count as an objective (lower is better)"""
    counts = []
    for circuit, valid in zip(circuits, is_valid):
        if not valid:
        ...
        gate_count = sum(1 for node in circuit.nodes if circuit.nodes[node].get("type") == "gate")
        counts.append(gate_count)
    return torch.tensor(counts)


def circuit2fidelity(circuits: List[QuantumCircuit], is_valid: List[bool], default=0):
    """Compute circuit fidelity as an objective (higher is better)"""
    # Placeholder for actual fidelity computation
    # In practice, this would involve:
    # 1. Converting the circuit to a quantum operation
    # 2. Computing fidelity with target operation
    fidelities = []
    for circuit, valid in zip(circuits, is_valid):
        if not valid:
            fidelities.append(default)
            continue
        # Placeholder: assume perfect fidelity for now
...
aux_tasks = {
    "depth": circuit2depth,
    "gate_count": circuit2gate_count,
    "fidelity": circuit2fidelity
}


class QuantumCircuitMOOTask(QuantumCircuitTask):
    """Sets up a multi-objective task for quantum circuits where the rewards are:
    - Circuit depth (minimize)
    - Total gate count (minimize)
    - Circuit fidelity (maximize)
    """
    ...
    def __init__(
        ...
        self.cfg = cfg
        mcfg = self.cfg.task.circuit_moo
        self.objectives = cfg.task.circuit_moo.objectives
        ...
        assert set(self.objectives) <= {"depth", "gate_count", "fidelity"} and len(self.objectives) == len(set(self.objectives))
    ...
    def compute_obj_properties(self, circuits: List[QuantumCircuit]) -> Tuple[ObjectProperties, Tensor]:
        is_valid = [True for _ in circuits]  # All circuits are valid for now
        ...
        if not any(is_valid):
            ...
            for obj in self.objectives:
                flat_r.append(aux_tasks[obj](circuits, is_valid))

            flat_rewards = torch.stack(flat_r, dim=1)
            assert flat_rewards.shape[0] == len(circuits)
            return ObjectProperties(flat_rewards), is_valid_t


class QuantumCircuitMOOTrainer(QuantumCircuitTrainer):
    task: QuantumCircuitMOOTask
    ctx: CircuitBuildingEnv
    ...
    def setup_task(self):
        self.cfg.cond.moo.num_objectives = len(self.cfg.task.circuit_moo.objectives)
        self.task = QuantumCircuitMOOTask(
            cfg=self.cfg,
    ...
    def setup_model(self):
        if self.cfg.algo.method == "MOQL":
            self.model = GraphTransformerFragEnvelopeQL(
                ...
                num_objectives=len(self.cfg.task.circuit_moo.objectives),
            )
        else:
            super().setup_model()
    ...
    def setup(self):
        super().setup()
        if self.cfg.task.circuit_moo.online_pareto_front:
        ...
        n_obj = len(self.cfg.task.circuit_moo.objectives)
        ...
        if isinstance(cond_cfg.focus_region.focus_type, list) and len(cond_cfg.focus_region.focus_type) > 1:
            ...
            n_valid = self.cfg.task.circuit_moo.n_valid
        ...
        self.test_data = RepeatedCondInfoDataset(valid_cond_vector, repeat=self.cfg.task.circuit_moo.n_valid_repeats)

        self._top_k_hook = TopKHook(10, self.cfg.task.circuit_moo.n_valid_repeats, n_valid)
        if self.cfg.task.circuit_moo.log_topk:
...
def main():
    ...
    config.desc = "debug_quantum_circuit_moo"
    config.log_dir = "./logs/debug_run_qcm"
    ...
    config.task.circuit_moo.objectives = ["depth", "gate_count", "fidelity"]
    ...
    config.task.circuit_moo.n_valid = 15
    config.task.circuit_moo.n_valid_repeats = 2

    trial = QuantumCircuitMOOTrainer(config)
    trial.run()
